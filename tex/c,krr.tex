...
leaving as few parameters as possible
to manual selection. 
In light of this goal,
we focus (where necessary)
on \emph{online} model selection,
where one chooses model parameters
for training the estimator $\esta{\bmx}{\cdot}$ 
\emph{after} observation of test data.
Though often impractical  
in traditional machine learning applications,
this approach is appropriate here
because typical voxel-wise separable
QMRI parameter estimation problems
are low dimensional relative
to literature machine learning applications,
allowing fast training.
...
If one requires offline training,
one must either assume
or empirically estimate 
from previous experiments
all of these distributions.
Here we instead focus 
on online training,
where one learns the estimator $\esta{\bmx}{\cdot}$ 
alongside data processing.
