...
\begin{comment}
	...
	\revone{maps (using maximum-likelihood (ML) and regularized least squares (RLS)} methods detailed in Section~\ref{sec:est}) from datasets collected using the scan profiles optimized in Section~\ref{sec:opt}. In Section~\ref{ss,scn-dsgn,exp:sim}, we study estimator statistics from simulated data.
	In Sections~\ref{ss,scn-dsgn,exp:phant}-\ref{ss,scn-dsgn,exp:invivo}, we progress to phantom and \emph{in vivo} datasets to evaluate scan profile performance and estimator robustness under increasingly complex settings. For the latter experiments, we use reference parameter maps from classical (long) pulse sequences, in lieu of ground truth maps.
...
\end{table}
	...
	In agreement with the predictions of $\costwt$ and $\costwb$, these simulation studies suggest that 
	\revtwo{
		at these SNR levels,
	}
