...
\begin{figure}
  ...
  \end{tikzpicture}
  \caption{
	Block diagram of greedy acquisition design.
...
% idea is to first optimize a small scan profile and 
...
for a fixed scan profile.
In Chapter~\ref{c,scn-dsgn},
we were able to restrict our time constraint
such that only three scan profiles
were feasible given timing restrictions,
each of which we studied explicitly.
Here,
our total time constraint is necessarily larger
and allows combinatorially many more scan combinations.
As an alternative
...
to compare all possible scan profiles exhaustively
...
For non-convex optimization problem \eqref{eq;mwf,P-hat},
the cost $\expcost\paren{\bmPc}$ 
of locally optimal solution $\bmPc$
is likely to depend on initialization.


Since $\expcost$ is non-convex in $\bmP$, 
the converged cost $\expcost{\bmPc}$ 
of 
