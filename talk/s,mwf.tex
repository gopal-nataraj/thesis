...
\begin{frame}{Previous MWF imaging acquisitions}
  ...
  \uncover<2>{%
	...
	\uncover<3->{%
		\textbf{Goal}: fast, accurate MWF quantification in WM
	}
\end{frame}
...
\begin{frame}{MWF Estimation via PERK}
	\uncover<1>{%
	...
	\uncover<2>{%
		\textbf{Use PERK to estimate just $\hlg{\ff}$}
		\begin{itemize}
...
\begin{frame}{MWF Simulation Result}
	...
	\uncover<+->{%
		\begin{figure}[t]
			\centering
			\includegraphics[width=\textwidth]{s,mwf/sim}%
		\end{figure}
	}
	\uncover<+->{%
...
\begin{frame}{MWF Proof-of-concept Experimental Study}
	...
	\uncover<2-3>{%
		...
		\begin{itemize}
			...
			\item<3->{KRR training and testing took \textbf{35s} and \textbf{5s/slice}}
		\end{itemize}
	}
	\uncover<4>{%
...
\begin{frame}{MWF Proof-of-concept In Vivo Result}
  ...
  \begin{figure}
  	...
  	\begin{minipage}[b]{0.64\textwidth}
  		\centering
  		\includegraphics [height=3.4cm, trim= 30 24 10 5, clip] {s,mwf/grase-mcdespot}
  	\end{minipage}
  	\begin{minipage}[b]{0.32\textwidth}
  		\centering
  		\includegraphics [height=3.4cm] {s,mwf/proposed}
  	\end{minipage}
	\end{figure}
\end{frame}
...
\begin{frame}{Summary}
	\uncover<1-2>{%
  	\textbf{Contributions}
  	\begin{itemize}
  		\item<1>{Fast acquisition for precise MWF estimation in WM}
			\item<1-2>{Proof-of-concept \invivo MWF images via PERK}
  	\end{itemize}
	}
	\uncover<2>{%
  	\textbf{Ongoing thesis work}
  	\begin{itemize}
	...
	\uncover<3->{%
		...
		\begin{itemize}
			\item<3>{%
				apply scan design and PERK to other QMRI applications,\\
				\eg MR fingerprinting \hfill \citec{nataraj:17:slw}
			}%
			\item<4>{%
				correlate our MWF estimates with other myelin biomarkers,\\
				\eg inhomogeneous mag transfer \hfill \citec{varma:15:mtf}
			}%
			\item<5>{%
				combine PERK and image reconstruction
			}%
		\end{itemize}
	}
\end{frame}
