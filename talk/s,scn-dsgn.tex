...
\begin{frame}{Signal Model}
	\alt<1>{%
		After reconstruction, single voxel $y_d$ in $d$th image modeled as
		\begin{align}
			y_d = s_d\paren{\bmx; \bmnu, \bmp_d} + \epsilon_d
		\end{align}
	}{%
		A \emph{scan profile} contains $D$ voxels $\bmy := \brac{y_1,\dots,y_D}\tpose$, 
			modeled as
			\begin{align}
	...
	\begin{itemize}
		\item<1>{\makebox[3.5cm][l]{$\bmx \in \reals{L}$} latent free parameters}
...
\begin{frame}{Scan Design}
	...
	\uncover<3->{%
  	...
  	\begin{itemize}
  		...
  		\item<4>{%
  			Bayesian scan design \hfill \textcolor{arch-ivy}{[\S6.3]}
  			\begin{align}
  				\breve{\bmP} &\in \set{
  					\argmin{\bmP \in \setP}
...
\begin{frame}{Scan Profile Comparison}
	\uncover<1>{%
      ...
      \begin{tabu} {r | c c c}
         ...
         \hline
         Optimized Cost & 4.0 & 4.9 & \textbf{3.5} \\
         \hline \hline
  		\end{tabu}}
			\label{table:profile}
  	\end{table}
...
\begin{frame}{Experimental Setup}
	...
	\uncover<3>{%
		...
		\begin{itemize}
			\item{Acquired 2 BS-shifted SPGR scans in 1m40s total}
...
\begin{frame}{Summary}
	\uncover<1-3>{%
		\textbf{Contributions}
			\begin{itemize}
				...
				\begin{itemize}
					\item<2->{%
						Simulation and phantom results validate method \\
	...
	\uncover<4>{%
		\textbf{How to address model mismatch?}
		\begin{itemize}	
			\item{More complete \invivo signal models}
